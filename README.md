...
closed-world evaluation, where attack families are represented during training

unseen attack-family evaluation, where an entire family is held out from training
...
attack-type and attack-family mapping

file-level experiment organization
...
create a stratified training-only subset

train XGBoost

calculate gain-based importance

calculate the median importance threshold

retain features meeting the threshold
...
GET /unseen-evaluation

Unseen-family results
...
Dataset discovery, labels, family distribution, quality checks

02

Preprocessing, splits, imbalance sampling

03

XGBoost gain feature selection
...
Reliability, threshold analysis and final research summary
...
established imbalance handling

39 → 20 feature reduction

tuned Gradient Boosting IDS

conventional closed-world evaluation

entire attack-family holdout

unseen-family evaluation

quantified degradation

false-positive/false-negative threshold analysis

interactive live prediction through FastAPI + React
...
Evaluation uses CICIoT2023 only.

Completed primary unseen-family experiments cover DDoS, DoS and Mirai.

The unseen-family setup is zero-day-like, not proof of universal real-world zero-day detection.

The default binary threshold produces a high false-positive rate.

Threshold sensitivity is not an independently selected deployment threshold.

LightGBM could not be evaluated because of a Windows native access-violation issue.

Streaming, concept drift and adversarial robustness were not fully evaluated.

The project uses flow-level tabular features rather than raw packet sequences.

CICIoT2023 is a benchmark dataset and may not capture every operational IoT environment.
